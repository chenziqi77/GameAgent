...
class ToolLoop:
    ...
    def run(self, *, system: str, user: str, candidate_ids: list[str]) -> ToolLoopResult:
        ...
        for rnd in range(1, self.max_rounds + 1):
            try:
                ...
                raw = self.llm.complete_json(system=system, user=user, temperature=0.1, max_tokens=self.max_tokens)
                return ToolLoopResult(decision_json=raw, tool_events=events, rounds=rnd, fallback=True)
            ...
            if not resp.tool_calls:
                ...
                if _valid_pick(obj, candidate_ids):
                    return ToolLoopResult(decision_json=obj, tool_events=events, rounds=rnd)
            ...
            for tc in resp.tool_calls:
                ...
                events.append({"name": name, "args": args, "ok": result.get("ok", False), "content_preview": (result.get("content") or "")[:600]})
                messages.append({
                    "role": "tool",
        ...
        if not _valid_pick(obj, candidate_ids):
            ...
            return ToolLoopResult(decision_json=obj, tool_events=events, rounds=self.max_rounds, fallback=True)
        return ToolLoopResult(decision_json=obj, tool_events=events, rounds=self.max_rounds)
    ...
    def _dispatch(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            return {"ok": result.ok, "content": result.to_model_output()}
        except Exception as exc:
            return {"ok": False, "content": json.dumps({"ok": False, "error": f"{type(exc).__name__}: {exc}"})}
